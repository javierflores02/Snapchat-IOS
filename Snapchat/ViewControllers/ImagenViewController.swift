...
class ImagenViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func elegirContactoTapped(_ sender: Any) {
        ...
        let cargarImagen = imagenesFolder.child("\(NSUUID().uuidString).jpg").putData(imagenData!, metadata: nil){(metadata, error) in
            if error != nil{
                self.mostrarAlerta(titulo: "Error", mensaje: "Se produjo u error al subir la imagen. Verifique su conexion a internet y vuelve a intentarlo.", accion: "Aceptar")
        ...
        cargarImagen.observe(.progress) {(snapshot) in
            ...
            alertaCarga.message = String(round(porcentaje*1000.0)) + "%"
            if porcentaje>=1.0{
